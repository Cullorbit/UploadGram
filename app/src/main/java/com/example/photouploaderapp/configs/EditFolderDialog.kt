...
class EditFolderDialog(
        ...
        dialog.setOnShowListener {
            dialog.getButton(android.app.AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                ...
                if (folderName.isBlank() || selectedFolderPath == null) {
                    ...
                    validateAndSave(folderName, topicId, dialog)
                }
            }
        }
    ...
    private fun loadTopics() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                client.newCall(Request.Builder().url(url).build()).execute().use { response ->
                    ...
                    if (ok) {
                        ...
                        if (topics != null && topics.length() > 0) {
                            ...
                            var currentTopicName = mainChatName
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                binding.tilManualTopicId.visibility = View.GONE
                                binding.etTopicNumber.setText(currentTopicName, false)
                        ...
                        withContext(Dispatchers.Main) {
                            if (description.contains("method not found", ignoreCase = true)) {
                                ...
                                binding.etManualTopicId.setText(folder.topic)
                            } else {
                                binding.tilTopicNumber.visibility = View.VISIBLE
                                binding.tilManualTopicId.visibility = View.GONE
                ...
                withContext(Dispatchers.Main) {
                    binding.tilTopicNumber.error = getString(R.string.network_error_loading_topics)
                }
            }
        }
    }

    private fun validateAndSave(folderName: String, topicId: String, dialog: Dialog) {
                ...
                client.newCall(request).execute().use { response ->
                    ...
                    if (topicId.isNotEmpty()) {
                        ...
                        client.newCall(topicRequest).execute().use { topicResponse ->
                            ...
                            withContext(Dispatchers.Main) {
                                if (topicOk) {
                                    saveFolderAndDismiss(folderName, topicId, dialog)
                                } else {
    ...
    private fun saveFolderAndDismiss(folderName: String, topicId: String, dialog: Dialog) {
